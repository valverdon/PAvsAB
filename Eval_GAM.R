...
Eval_GAM<-function(PAAB,arrayID,GtoM,ENVdata, Model_list, NbRunEval=100, validation.method = "bootstrap", DataSplit=0.8, test=FALSE){
  #Model_list<-Modlist
  #for gam
  library(mgcv)
  ...
  # load(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Models/Models_temp",arrayID,".Rda"))
  # NbRunEval=100
  ...
  for (i in 1:length(Model_list)){#i=1
    ...
    if(PAAB=="PA"){
      if(all(is.na(Mod))){#dont bother evaluate if no model
        ...
        load(paste0(PAAB,"/",GtoM,"/Outputs/GAM/VarSel/VarSel_temp",arrayID,".Rda"))
        ...
        for (f in 1:ncol(dataSplits)) { #f=1 for the different selection of traning/eval plots.
          ...
          formCV <- as.formula(paste0("OTUtrain~ ", paste(Mod$formula)[3]))
          ModEval <- tryCatch(
          ...
          if(is.na(ModEval[1])){
            ...
            predCV <- predict.gam(ModEval, newdata = ENVeval, type = "response")
            pred_GAM[rownames(ENVeval), paste0("pred",f)] <- predCV  
            if(validation.method!="LOO"){
              tryCatch(
                { Evalvalues[paste0("pred",f),] <- Evalmetrics(obs=as.numeric(OTUeval),pred=predCV,PAAB="PA") #offset log bcz nb family (select = T to flatten smoothers of non important variables)
        ...
        if(validation.method=="LOO"){
          pred_GAM2<-apply(pred_GAM,1,function(X){mean(X,na.rm=TRUE)})
          tryCatch(
            { Evalvalues[paste0("pred",f),] <- Evalmetrics(obs=OTUdata[,OTUtoRun],pred=pred_GAM2,PAAB="PA") #offset log bcz nb family (select = T to flatten smoothers of non important variables)
            }, error=function(cond){#get D2 for the projection on the non-selected sites of the boot.
              message(paste0("error evaluating boot ",f," of OTU ", OTUtoRun))
              message(paste("boot will be ignored"))
              return(Evalvalues[paste0("pred",f),] <- rep(NA, 15))
      ...
      }#end elseodel exist for that ASV
    }#PA
    ...
    if(PAAB=="AB"){
      ...
      if (is.na(Mod[1])){
        ...
        for (f in 1:ncol(dataSplits)) { #f=1 for the different selection of traning/eval plots.
          ...
          formCV <- as.formula(paste0("OTUtrain~ ", gsub("TotSeqSum","TotSeqSumtrain",paste(summary(Mod)$formula)[3])))
          ModEval <- tryCatch(
          ...
          if(is.na(ModEval[1])){
            ...
            predCV <- predict.gam(ModEval, newdata=data.frame(ENVeval, TotSeqSumtrain = log(TotSeqSum[dataSplits[ ,f]==FALSE])), type="response")
            pred_GAM[rownames(ENVeval), paste0("pred",f)] <- predCV  
            if(validation.method!="LOO"){
              tryCatch(
      ...
      if(validation.method=="LOO"){
        pred_GAM2<-apply(pred_GAM,1,function(X){mean(X,na.rm=TRUE)})
        tryCatch(
          { Evalvalues[paste0("pred",f),] <- Evalmetrics(obs=OTUdata[,OTUtoRun],pred=pred_GAM2,PAAB="AB") #offset log bcz nb family (select = T to flatten smoothers of non important variables)
          }, error=function(cond){#get D2 for the projection on the non-selected sites of the boot.
            message(paste0("error evaluating boot ",f," of OTU ", OTUtoRun))
            message(paste("boot will be ignored"))
            return(Evalvalues[paste0("pred",f),] <- rep(NA, 15))
      ...
      pred_eval <- data.frame(obs=OTUdata[,OTUtoRun])
      # cbind(pred_eval,pred_GAM)[pred_GAM[,1]==max(pred_GAM[,1],na.rm=TRUE),c(1,2)]
      # max(pred_GAM,na.rm=TRUE)
      Pred_data_list[[paste0("OTU",OTUtoRun)]]  <- cbind(pred_eval,pred_GAM)
      Eval_met_list[[paste0("OTU",OTUtoRun)]] <- apply(Evalvalues, 2, function(X){median(X,na.rm=T)})#median of all boots (avoid -inf problems that would appear with mean)
      Eval_values_list[[paste0("OTU",OTUtoRun)]] <- Evalvalues
    }#end AB
  ...
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met"))){
    save(Eval_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met/Eval_met_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met"))
    save(Eval_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met/Eval_met_temp", arrayID, ".Rda"))
  }
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Pred_data"))){
    save(Pred_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GAM/Pred_data/Pred_data_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Pred_data"))
    save(Pred_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GAM/Pred_data/Pred_data_temp", arrayID, ".Rda"))
  }
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met_allboot/"))){
    save(Eval_values_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met_allboot/Eval_met_allboot_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met_allboot"))
    save(Eval_values_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GAM/Eval_met_allboot/Eval_met_allboot_temp", arrayID, ".Rda"))
  }
  if(test){
    return(Eval_met_mat)
