...
Eval_GBM<-function(PAAB,arrayID,GtoM,ENVdata, Model_list, NbRunEval=100, validation.method = "bootstrap",DataSplit=0.8, test=FALSE){
  #Model_list<-Modlist
  #for gam
  library(gbm)
    ...
    if(PAAB=="PA"){
      if(all(is.na(Mod))){#dont bother evaluate if no model
        ...
        for (f in 1:ncol(dataSplits)) { #f=1 for the different selection of traning/eval plots.
          ...
          ModEval <- tryCatch(
            { gbm(formCV, data=as.data.frame(ENVtrain), distribution="bernoulli", weights = weights, n.cores= nthreads, n.trees=Mod$n.trees,shrinkage=Mod$shrinkage,bag.fraction=Mod$bag.fraction)
            }, error=function(cond){
  ...
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met"))){
    save(Eval_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met/Eval_met_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met"))
    save(Eval_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met/Eval_met_temp", arrayID, ".Rda"))
  }
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Pred_data"))){
    save(Pred_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Pred_data/Pred_data_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Pred_data"))
    save(Pred_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Pred_data/Pred_data_temp", arrayID, ".Rda"))
  }
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met_allboot/"))){
    save(Eval_values_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met_allboot/Eval_met_allboot_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met_allboot"))
    save(Eval_values_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Eval_met_allboot/Eval_met_allboot_temp", arrayID, ".Rda"))
  }
  if(test){
    return(Eval_met_mat)
