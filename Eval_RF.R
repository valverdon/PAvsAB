...
Eval_RF<-function(PAAB,arrayID,GtoM,ENVdata, Model_list, NbRunEval=100, DataSplit=0.8, validation.method = "bootstrap",test=FALSE){
  #Model_list<-Modlist
  #for gam
    ...
    if(PAAB=="PA"){
      if(all(is.na(Mod))){#dont bother evaluate if no model
        ...
        for (f in 1:ncol(dataSplits)) { #f=1 for the different selection of traning/eval plots.
          ...
          #################################
          ModEval <- tryCatch(
            { RRF(formCV, data=as.data.frame(ENVtrain), ntree=Mod$ntree, classwt=c("FALSE"=unique(weights[OTUtrain==FALSE]),"TRUE"=unique(weights[OTUtrain==TRUE])))
            }, error=function(cond){
  ...
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met"))){
    save(Eval_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met/Eval_met_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met"))
    save(Eval_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met/Eval_met_temp", arrayID, ".Rda"))
  }
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Pred_data"))){
    save(Pred_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Pred_data/Pred_data_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Pred_data"))
    save(Pred_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Pred_data/Pred_data_temp", arrayID, ".Rda"))
  }
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met_allboot/"))){
    save(Eval_values_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met_allboot/Eval_met_allboot_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met_allboot"))
    save(Eval_values_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Eval_met_allboot/Eval_met_allboot_temp", arrayID, ".Rda"))
  }
  if(test){
    return(Eval_met_mat)
