...
edaphic<-c("pH","pH.1","ndvi","bulkSoilW","soilTemp","EC_1_5",    "TotalP",    "Nitrogen",  "Carbon",    "Hydrogen",  "Phyllosil", "Quartz",    "Feldspath","Plagiocla","MassiveLi",
           ...
           "K2O" ,  "P2O5"  , "OM"  ,  "Cr2O3",  "NiO",   "d15N" ,  "d13C","Silt_clay", "clay"  , "ThinSilt", "ThickSilt", "ThinSand", "ThickSand","Soil_aera","Soil_humu","Soil_mois","Soil_mois.1","Soil_nutr" ,"MoistVar")
topographic<-c("aspect","slope","Elevation","Altitude")
landcover<-c("forest_ag", "hydro_agg", "lowVeg_ag", "anthropos",   "deciduous")
noise<-"noise"
col_table<-data.frame(categ=c("climatic","edaphic","landcover","ndmi","topographic","noise"),color=c("#F5E6A4",'#F5BBAE',"#8CF5CE",'#457EB0','gray69',"#BC81C8"))

Prop_models_one_cov<-matrix(NA,ncol=6,nrow=2*4*4)
colnames(Prop_models_one_cov)<-c("climatic","edaphic","landcover","ndmi","topographic","noise")
  ...
  for(algo in c("GLM","GAM","GBM","RF")){#Pb empty Variable_ranks.Rda GAM+GBM PA --> Bug Figure Selected Var
    ...
    for(GtoM in c("PR","FU","BA")){
      ...
      if(GtoM=="BA"){
        ...
        allpresel3$categ<-factor(allpresel3$categ,levels=c("climatic","edaphic","landcover","ndmi","topographic","noise"))
        ...
        allsel3$categ<-factor(allsel3$categ,levels=c("climatic","edaphic","landcover","ndmi","topographic","noise"))
        ...
        allsel4<-apply(Variable_importance1*allsel_BA,2,function(X){na.exclude(X)})
        # summary(allsel_BA2)
        ...
        data_reordered4$categ<-factor(data_reordered4$categ,levels=c("climatic","edaphic","landcover","ndmi","topographic","noise"))
        ...
        assign(paste0("pVIMP10_BA_",algo,"_",PAAB),ggplot(data_reordered4_10,aes(x=L1,y=value,fill=categ))+theme_bw()+geom_boxplot()+ xlab("") + ylab(ifelse(PAAB=="AB","","Bacteria"))  +
                 ...
                 scale_y_continuous(limits=c(0,1)))
        #pVIMP_BA_GLM_PA
        sum(allsel_BA[1,][names(allsel_BA[1,])%in%climatic],na.rm=TRUE)
        allsel5_temp<-apply(allsel_BA,1,function(X){c(sum(X[names(X)%in%climatic],na.rm=TRUE)>=1,sum(X[names(X)%in%edaphic],na.rm=TRUE)>=1,sum(X[names(X)%in%landcover],na.rm=TRUE)>=1, sum(X[names(X)=="ndmi"],na.rm=TRUE)>=1, sum(X[names(X)%in%topographic],na.rm=TRUE)>=1,sum(X[names(X)=="noise"],na.rm=TRUE)>=1)})
        allsel5_temp<-t(allsel5_temp)
        colnames(allsel5_temp)<-c("climatic","edaphic","landcover","ndmi","topographic","noise")
        ...
        # boxplot(Variable_importance2,main=paste0(PAAB,"_",GtoM,"_",algo,"_Varimp_distribution_standard_all"),las = 2)
        allsel4<-apply(Variable_importance1*allsel_AR,2,function(X){na.exclude(X)})
        # boxplot(allsel4,main=paste0(PAAB,"_",GtoM,"_",algo,"_Varimp_distribution_standard_only_when_selected"),las = 2)
        # summary(allsel_BA2)
        # allsel4<-Variable_importance2[Variable_importance2==0]
        ...
        assign(paste0("pVIMP10_AR_",algo,"_",PAAB),ggplot(data_reordered4_10,aes(x=L1,y=value,fill=categ))+theme_bw() +geom_boxplot()+ xlab("") + ylab(ifelse(PAAB=="AB","","Archaea"))  +
                 ...
                 scale_y_continuous(limits=c(0,1)))
        #pVIMP_AR_GLM_PA
        allsel5_temp<-apply(allsel_AR,1,function(X){c(any(X[names(X)%in%climatic]>=1),any(X[names(X)%in%edaphic]>=1),any(X[names(X)%in%landcover]>=1), any(X[names(X)=="ndmi"]>=1), any(X[names(X)%in%topographic]>=1), any(X[names(X)=="noise"]>=1))})
        allsel5_temp<-t(allsel5_temp)
        colnames(allsel5_temp)<-c("climatic","edaphic","landcover","ndmi","topographic","noise")
        allsel5_temp<-apply(allsel5_temp,2,function(X){sum(X,na.rm=TRUE)/length(X)})
      ...
      if(GtoM %in% c("PR","FU")){ #GtoM="FU"
        ...
        # colnames(allsel_group)<-colnames(allsel_group)
        ...
        allsel2<-apply(allsel[,colnames(allsel)%in%colnames(ENVdata)|colnames(allsel)=="Altitude"],2,function(X){sum(X,na.rm=TRUE)/length(X)})
        # allsel2["Soil_mois"]
        # barplot(allsel2,main=paste0(PAAB,"_",GtoM,"_",algo,"_Selection"),las = 2)
        allsel2<-allsel2[order(allsel2,decreasing=TRUE)]
        #proportion of models in which variable selected
        ...
        # summary(Variable_importance1)
        # Variable_importance2<-matrix(NA,ncol=length(allsel2),nrow=nrow(Variable_importance1))
        # rownames(Variable_importance2)<-rownames(Variable_importance1)
        # colnames(Variable_importance2)<-names(allsel2)
        # for (i in 1:length(allsel2)){#i=1
        #   if(length(groups[[i]])>1){
        #     Variable_importance2[,groups[[i]][1]]<-apply(Variable_importance1[,colnames(Variable_importance1)%in%groups[[i]]],1,function(X){ifelse(all(is.na(X)),NA,sum(X,na.rm=TRUE))})
        #     Variable_importance2[,groups[[i]][1]]<-Variable_importance1[,colnames(Variable_importance1)%in%groups[[i]]]
        #   }
        ...
        # boxplot(Variable_importance2,main=paste0(PAAB,"_",GtoM,"_",algo,"_Varimp_distribution_standard_all"),las = 2)
        allsel4<-apply(Variable_importance1*allsel,2,function(X){na.exclude(X)})
        ...
        allsel5_temp<-apply(allsel,1,function(X){c(any(X[names(X)%in%climatic]>=1),any(X[names(X)%in%edaphic]>=1),any(X[names(X)%in%landcover]>=1), any(X[names(X)=="ndmi"]>=1), any(X[names(X)%in%topographic]>=1), any(X[names(X)=="noise"]>=1))})
        allsel5_temp<-t(allsel5_temp)
        colnames(allsel5_temp)<-c("climatic","edaphic","landcover","ndmi","topographic","noise")
        allsel5_temp<-apply(allsel5_temp,2,function(X){sum(X,na.rm=TRUE)/length(X)})
...
pdf(file=paste0("figures/PAAB_selection/VARimp_nogr/All_varimp_Presel_GLMGAMGBMRF.pdf"))
plot(Ppresel_GLM)
plot(Ppresel_GAM)
plot(Ppresel_GBM)
plot(Ppresel_RF)
...
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GLM_Presel.png"),res=300,width=1961,height=1500)
plot(Ppresel_GLM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GAM_Presel.png"),res=300,width=1961,height=1500)
plot(Ppresel_GAM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GBM_Presel.png"),res=300,width=1961,height=1500)
plot(Ppresel_GBM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/RF_Presel.png"),res=300,width=1961,height=1500)
plot(Ppresel_RF)
dev.off()

pdf(file=paste0("figures/PAAB_selection/VARimp_nogr/All_varimp_Sel_GLMGAMGBMRF.pdf"))
plot(Psel_GLM)
plot(Psel_GAM)
plot(Psel_GBM)
plot(Psel_RF)
...
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GLM_Sel.png"),res=300,width=1961,height=1500)
plot(Psel_GLM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GAM_Sel.png"),res=300,width=1961,height=1500)
plot(Psel_GAM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GBM_Sel.png"),res=300,width=1961,height=1500)
plot(Psel_GBM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/RF_Sel.png"),res=300,width=1961,height=1500)
plot(Psel_RF)
dev.off()

pdf(file=paste0("figures/PAAB_selection/VARimp_nogr/All_varimp_Sel10_GLMGAMGBMRF.pdf"))
plot(Psel10_GLM)
plot(Psel10_GAM)
plot(Psel10_GBM)
plot(Psel10_RF)
...
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GLM_Sel10.png"),res=300,width=1961,height=1500)
plot(Psel10_GLM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GAM_Sel10.png"),res=300,width=1961,height=1500)
plot(Psel10_GAM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GBM_Sel10.png"),res=300,width=1961,height=1500)
plot(Psel10_GBM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/RF_Sel10.png"),res=300,width=1961,height=1500)
plot(Psel10_RF)
dev.off()



pdf(file=paste0("figures/PAAB_selection/VARimp_nogr/All_varimp_VIMP_GLMGAMGBMRF.pdf"))
plot(PVIMP_GLM)
plot(PVIMP_GAM)
plot(PVIMP_GBM)
plot(PVIMP_RF)
...
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GLM_VIMP.png"),res=300,width=1961,height=1500)
plot(PVIMP_GLM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GAM_VIMP.png"),res=300,width=1961,height=1500)
plot(PVIMP_GAM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GBM_VIMP.png"),res=300,width=1961,height=1500)
plot(PVIMP_GBM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/RF_VIMP.png"),res=300,width=1961,height=1500)
plot(PVIMP_RF)
dev.off()

pdf(file=paste0("figures/PAAB_selection/VARimp_nogr/All_varimp_VIMP10_GLMGAMGBMRF.pdf"))
plot(PVIMP10_GLM)
plot(PVIMP10_GAM)
plot(PVIMP10_GBM)
plot(PVIMP10_RF)
...
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GLM_VIMP10.png"),res=300,width=1961,height=1500)
plot(PVIMP10_GLM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GAM_VIMP10.png"),res=300,width=1961,height=1500)
plot(PVIMP10_GAM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GBM_VIMP10.png"),res=300,width=1961,height=1500)
plot(PVIMP10_GBM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/RF_VIMP10.png"),res=300,width=1961,height=1500)
plot(PVIMP10_RF)
dev.off()

png(file=paste0("figures/PAAB_selection/VARimp_nogr/GLM_groups.png"),res=300,width=1961,height=1500)
plot(Pgrouped_GLM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GAM_groups.png"),res=300,width=1961,height=1500)
plot(Pgrouped_GAM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/GBM_groups.png"),res=300,width=1961,height=1500)
plot(Pgrouped_GBM)
dev.off()
png(file=paste0("figures/PAAB_selection/VARimp_nogr/RF_groups.png"),res=300,width=1961,height=1500)
plot(Pgrouped_RF)
dev.off()
