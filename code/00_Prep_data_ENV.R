...
ENVstack<-rast("../../spatial_data/ENVstack.rds")
...
for (group in c("BA","FU","PR")){#group="BA"
  ...
  readcounts<-readRDS(file=paste0("../../ASV_data/readcounts_",group,"_Guex.Rds"))#readcount per site per ASV
  ...
  ENVdatatemp <- ENVdatatemp[,-which(colnames(ENVdatatemp)%in%c("drySoilWeight","wetSoilWeight","EC_1_1","AllSand","AllSilt","Tmax","C.N","TOC","MINC","ID","north1","pH_modeled"))]
  ...
  saveRDS(ENVdata,file=paste0("PA/",group,"/data/ENVdata_",group,".Rds"))
  saveRDS(ENVdata,file=paste0("AB/",group,"/data/ENVdata_",group,".Rds"))
}

#Next code : 00_Pred_data_OTU
