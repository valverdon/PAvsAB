...
Eval_GLM<-function(PAAB,arrayID,GtoM,ENVdata,OTUdata,OTUstoRun, Model_list, NbRunEval=100, DataSplit=0.8, validation.method = "bootstrap",CCV=FALSE,test=FALSE){
  #for GLM
  library(data.table)
  library(stringi)
  library(glmnet)
  ...
  for (OTUtoRun in OTUstoRun){#OTUtoRun=OTUstoRun[1]
    ...
    if(CCV){ #if community cross validation, get common CV setup, else create CV setup for this OTU.
      dataSplits<-readRDS(paste0("data/splitOTUdata_",validation.method,"_",NbRunEval,"splits_",GtoM,".Rds"))
    }else{
...
#     }#end AB
  ...
  if(CCV){
    ...
    if (file.exists(paste0("data/Outputs/GLM/",PAAB,"/Eval_met"))){
      save(Eval_met_mat, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met/Eval_met_CCV_temp_", arrayID, ".Rda"))
    } else {
      dir.create(paste0("data/Outputs/GLM/",PAAB,"/Eval_met"))
      save(Eval_met_mat, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met/Eval_met_CCV_temp_", arrayID, ".Rda"))
    }
    if (file.exists(paste0("data/Outputs/GLM/",PAAB,"/Pred_data"))){
      save(Pred_data_list, file=paste0("data/Outputs/GLM/",PAAB,"/Pred_data/Pred_data_CCV_temp_", arrayID, ".Rda"))
    } else {
      dir.create(paste0("data/Outputs/GLM/",PAAB,"/Pred_data"))
      save(Pred_data_list, file=paste0("data/Outputs/GLM/",PAAB,"/Pred_data/Pred_data_CCV_temp_", arrayID, ".Rda"))
    }
    if (file.exists(paste0("data/Outputs/GLM/",PAAB,"/Eval_met_allboot/"))){
      save(Eval_values_list, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met_allboot/Eval_met_CCV_allboot_temp_", arrayID, ".Rda"))
    } else {
    ...
    if (file.exists(paste0("data/Outputs/GLM/",PAAB,"/Eval_met"))){
      save(Eval_met_mat, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met/Eval_met_temp_", arrayID, ".Rda"))
    } else {
      dir.create(paste0("data/Outputs/GLM/",PAAB,"/Eval_met"))
      save(Eval_met_mat, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met/Eval_met_temp_", arrayID, ".Rda"))
    }
    if (file.exists(paste0("data/Outputs/GLM/",PAAB,"/Pred_data"))){
      save(Pred_data_list, file=paste0("data/Outputs/GLM/",PAAB,"/Pred_data/Pred_data_temp_", arrayID, ".Rda"))
    } else {
      dir.create(paste0("data/Outputs/GLM/",PAAB,"/Pred_data"))
      save(Pred_data_list, file=paste0("data/Outputs/GLM/",PAAB,"/Pred_data/Pred_data_temp_", arrayID, ".Rda"))
    }
    if (file.exists(paste0("data/Outputs/GLM/",PAAB,"/Eval_met_allboot/"))){
      save(Eval_values_list, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met_allboot/Eval_met_allboot_temp_", arrayID, ".Rda"))
    } else {
      dir.create(paste0("data/Outputs/GLM/",PAAB,"/Eval_met_allboot"))
      save(Eval_values_list, file=paste0("data/Outputs/GLM/",PAAB,"/Eval_met_allboot/Eval_met_allboot_temp_", arrayID, ".Rda"))
    }
