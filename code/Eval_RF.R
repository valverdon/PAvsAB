...
Eval_RF<-function(PAAB,arrayID,GtoM,ENVdata,OTUdata,OTUstoRun, Model_list, NbRunEval=100, DataSplit=0.8, validation.method = "bootstrap",test=FALSE){
  ...
  # Model_list<-readRDS(paste0("data/Outputs/RF/PA/Models/Models_BA_temp_1.rds"))
  # 
  # NbRunEval=100
  ...
  ENVdata[binvar] <- lapply(ENVdata[binvar],factor)
  ENVdata[contvar] <- lapply(ENVdata[contvar],as.numeric)
  ...
  for (OTUtoRun in OTUstoRun){#OTUtoRun=OTUstoRun[1] #OTUstoRun=10:15 #OTUtoRun=12
  ...
  if(CCV){
    if (file.exists(paste0("data/Outputs/RF/",PAAB,"/Eval_met"))){
      saveRDS(Eval_met_mat, file=paste0("data/Outputs/RF/",PAAB,"/Eval_met/Eval_met_CCV_temp_", arrayID, ".Rds"))
    } else {
      dir.create(paste0("data/Outputs/RF/",PAAB,"/Eval_met"))
      save(Eval_met_mat, file=paste0("data/Outputs/RF/",PAAB,"/Eval_met/Eval_met_CCV_temp_", arrayID, ".Rds"))
    }
    if (file.exists(paste0("data/Outputs/RF/",PAAB,"/Pred_data"))){
      save(Pred_data_list, file=paste0("data/Outputs/RF/",PAAB,"/Pred_data/Pred_data_CCV_temp_", arrayID, ".Rds"))
    } else {
      dir.create(paste0("data/Outputs/RF/",PAAB,"/Pred_data"))
      save(Pred_data_list, file=paste0("data/Outputs/RF/",PAAB,"/Pred_data/Pred_data_CCV_temp_", arrayID, ".Rds"))
    }
    if (file.exists(paste0("data/Outputs/RF/",PAAB,"/Eval_met_allboot/"))){
      save(Eval_values_list, file=paste0("data/Outputs/RF/",PAAB,"/Eval_met_allboot/Eval_met_CCV_allboot_temp_", arrayID, ".Rds"))
    } else {
