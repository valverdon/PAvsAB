Fit_GBM<-function(PAAB,arrayID,GtoM,OTUdata,ENVdata,test){
  ...
  Hyper.gbm<-function(form,data,distribution="poisson",weights,n.cores,n.trees.range=c(10,100,1000,10000),shrinkage.range=c(0.001,0.01,0.1)){
    ...
    for(ntrees in 1:length(n.trees.range)){#ntrees=1
      for(shrink in 1:length(shrinkage.range)){#shrink=1
        resultsMatrix[ntrees,shrink] <- tryCatch(
            {
              cvMod <- gbm(form, data=data, distribution=distribution, weights = weights,n.cores= n.cores,n.trees=n.trees.range[ntrees],shrinkage=shrinkage.range[shrink],bag.fraction=0.5,cv.fold=10)
  ...
  for (i in loop){ #i=1
    ...
    if(PAAB=="PA"){
      if (sum(OTUdata[,OTUtoRun])/length(OTUdata[,OTUtoRun])>0.95) {#dont bother model it if no absence data (more than 95% presence over all plots)
        ...
        hyperparams <- Hyper.gbm(form=form,data=ENVdata,distribution="bernoulli",weights=weights,n.cores=nthreads,n.trees.range=c(10,100,1000,10000),shrinkage.range=c(0.001,0.01,0.1))
        
        
        Mod  <- tryCatch(
          {
            gbm(form, data=ENVdata, distribution="bernoulli", weights = weights,n.cores= nthreads,n.trees=hyperparams$best_ntrees[1],shrinkage=hyperparams$best_shrink[1],bag.fraction=0.5)
          }, error=function(cond){
  ...
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/UnivExpl"))){
    save(UnivExpl_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/UnivExpl/UnivExpl_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM"))
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/UnivExpl"))
    save(UnivExpl_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/UnivExpl/UnivExpl_temp", arrayID, ".Rda"))
  }
  #Selected Variables and shrinkage
  VarSel <- list(preselection=preselected_list,ranking=ranking_list)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/VarSel"))){
    save(VarSel, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/VarSel/VarSel_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/VarSel"))
    save(VarSel, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/VarSel/VarSel_temp", arrayID, ".Rda"))
  }
  #Model Deviances
  Deviances_mat <- do.call("rbind", Deviances_list)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Deviances"))){
    save(Deviances_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Deviances/Deviances_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Deviances"))
    save(Deviances_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Deviances/Deviances_temp", arrayID, ".Rda"))
  }
  #Model calibration metrics
  Fit_met_mat <- do.call("rbind", Fit_met_list)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_met"))){
    save(Fit_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_met/Fit_met_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_met"))
    save(Fit_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_met/Fit_met_temp", arrayID, ".Rda"))
  }
  #save fit and cross validation models data (if needed)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_data"))){
    save(Fit_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_data/Fit_data_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_data"))
    save(Fit_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Fit_data/Fit_data_temp", arrayID, ".Rda"))
  }
  #Save model fit
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Models"))){
    save(Model_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Models/Models_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/GBM/Models"))
    save(Model_list, file=paste0(PAAB,"/",GtoM,"/Outputs/GBM/Models/Models_temp", arrayID, ".Rda"))
  }
  return(Model_list)
}#end function
