...
Fit_RF<-function(PAAB,arrayID,GtoM,OTUdata,ENVdata,test=FALSE){
  #for RF
  # set.seed(seed=08022022)
  # PAAB=PAAB; arrayID=arrayID; GtoM=GtoM; ENVdata=ENVdata; test=TRUE
  library(randomForest)
  library(RRF)
  Hyper.rf<-function(form,data,weights=NA,n.trees.range=c(10,100,1000,10000),PAAB){
    ...
    for(ntrees in 1:length(n.trees.range)){#ntrees=1
      for (i in 1:100){
        modqual[i,ntrees]<-tryCatch(
          {
        cvMod<-RRF(form, data=data, replace=TRUE, classwt = weights,n.trees=n.trees.range[ntrees])
  ...
  for (i in loop){ #i=loop[3]
    ...
    if(PAAB=="PA"){
      if (sum(OTUdata[,OTUtoRun])/length(OTUdata[,OTUtoRun])>0.95) {#dont bother model it if no absence data (more than 95% presence over all plots)
        ...
        ntree<-Hyper.rf(PAAB=PAAB, form=form,data=ENVdata,classwt,n.trees.range=c(10,100,1000,10000))
        #fit regularized random forest model.
        MRF<-RRF(form,data=ENVdata,replace=TRUE,ntree=ntree[1],classwt=classwt)
  ...
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/UnivExpl"))){
    save(UnivExpl_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/UnivExpl/UnivExpl_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF"))
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/UnivExpl"))
    save(UnivExpl_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/UnivExpl/UnivExpl_temp", arrayID, ".Rda"))
  }
  #Selected Variables and shrinkage
  VarSel <- list(preselection=preselected_list,ranking=ranking_list)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/VarSel"))){
    save(VarSel, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/VarSel/VarSel_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/VarSel"))
    save(VarSel, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/VarSel/VarSel_temp", arrayID, ".Rda"))
  }
  #Model Deviances
  Deviances_mat <- do.call("rbind", Deviances_list)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Deviances"))){
    save(Deviances_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Deviances/Deviances_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Deviances"))
    save(Deviances_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Deviances/Deviances_temp", arrayID, ".Rda"))
  }
  #Model calibration metrics
  Fit_met_mat <- do.call("rbind", Fit_met_list)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_met"))){
    save(Fit_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_met/Fit_met_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_met"))
    save(Fit_met_mat, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_met/Fit_met_temp", arrayID, ".Rda"))
  }
  #save fit and cross validation models data (if needed)
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_data"))){
    save(Fit_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_data/Fit_data_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_data"))
    save(Fit_data_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Fit_data/Fit_data_temp", arrayID, ".Rda"))
  }
  #Save model fit
  if (file.exists(paste0(PAAB,"/",GtoM,"/Outputs/RF/Models"))){
    save(Model_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Models/Models_temp", arrayID, ".Rda"))
  } else {
    dir.create(paste0(PAAB,"/",GtoM,"/Outputs/RF/Models"))
    save(Model_list, file=paste0(PAAB,"/",GtoM,"/Outputs/RF/Models/Models_temp", arrayID, ".Rda"))
  }
  return(Model_list)
}#end function
