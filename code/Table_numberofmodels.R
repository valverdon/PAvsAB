...
nASV_PR<-c(length(evalmetGLM$TSS),
           ...
           length(evalmetRF$TSS)) #2155
#number of modelled (crashed models and prev>0.95 excluded)
nMod_PR<-c(sum(!(is.na(evalmetGLM$TSS))),#2014
           sum(!(is.na(evalmetGAM$TSS))),#2138
           sum(!(is.na(evalmetGBM$TSS))),#2147
           sum(!(is.na(evalmetRF$TSS)))) #2147
