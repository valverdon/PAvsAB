...
###################################################
CovImp_GLM_PA<-read.csv(file="figures/PAAB_selection/Figshare_tables/CovImp_GLM_PA.csv",sep=";")
CovImp_GLM_PA_bin<-CovImp_GLM_PA[-which(colnames(CovImp_GLM_PA)%in%c("X","seq","Phylum","marker"))]>0
nOTUsel_cov<-matrix(data=NA,nrow=length(unique(CovImp_GLM_PA[,"Phylum"])),ncol=ncol(CovImp_GLM_PA_bin))#init matrix
colnames(nOTUsel_cov)<-colnames(CovImp_GLM_PA_bin)
rownames(nOTUsel_cov)<-unique(CovImp_GLM_PA[,"Phylum"])
CovImp_GLM_PA_bin<-cbind(CovImp_GLM_PA[which(colnames(CovImp_GLM_PA)%in%c("X","seq","Phylum","marker"))],CovImp_GLM_PA_bin)

for (Phylum in (unique(CovImp_GLM_PA_bin[,"Phylum"]))){#Phylum=unique(CovImp_GLM_PA_bin[,"Phylum"])[1]
  CovImp_GLM_PA_P<-CovImp_GLM_PA_bin[CovImp_GLM_PA_bin$Phylum==Phylum,-which(colnames(CovImp_GLM_PA_bin)%in%c("X","seq","Phylum","marker"))]
  nOTUsel_cov[Phylum,]<-apply(CovImp_GLM_PA_P,2,function(x){sum(x,na.rm=TRUE)})
  barplot(nOTUsel_cov[1,order(nOTUsel_cov[1,],decreasing=TRUE)][1:15],main=Phylum)
}
